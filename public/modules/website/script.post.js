$(document).ready(function () {
    ...
    var _table = $('.on-table-post').DataTable({
        ajax: '/website/post/ajax',
        language: {
    ...
    $('.on-contact-dropdown').dropdown({
        apiSettings: {
            url: '/website/post/select-contact-dropdown?q={query}',
            cache: false
        },
        clearable: true,
    ...
    $('.on-table-post').on('click', '.on-button-update', function () {
        var row = _table.row($(this).parents('tr')).data();

        $.ajax({
            type: 'GET',
            url: '/website/post/select',
            data: {q: row[ 0 ]},
            success: function (e) {
    ...
    $('.on-table-post').on('click', '.on-button-delete', function () {
        var row = _table.row($(this).parents('tr')).data();
