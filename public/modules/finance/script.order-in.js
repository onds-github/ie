$(document).ready(function () {
    ...
    var _table = $('.on-table-order-in').DataTable({
        ajax: '/finance/order-in/ajax',
        language: {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                orderable: false,
                render: function (data, type, row) {
                    return (parseInt(row[7]) == 1 ? '<button class="circular ui icon green button on-button-not-okay" uk-tooltip="title: Desfazer Baixa"><i class="thumbs up icon"></i></button>' : '<button class="circular ui icon button on-button-okay" uk-tooltip="title: Baixar Recebimento"><i class="thumbs down icon"></i></button>') +
    ...
    $('.on-contact-dropdown').dropdown({
        apiSettings: {
            url: '/finance/order-in/select-contact-dropdown?q={query}',
            cache: false
        },
    ...
    $('.on-account-dropdown').dropdown({
        apiSettings: {
            url: '/finance/order-in/select-account-dropdown?q={query}',
            cache: false
        },
        filterRemoteData: true,
        clearable: true
    ...
    $('.on-cost-center-dropdown').dropdown({
        apiSettings: {
            url: '/finance/order-in/select-cost-center-dropdown?q={query}',
            cache: false
        },
        filterRemoteData: true,
        clearable: true
    ...
    $('.on-table-order-in').on('click', '.on-button-duplicate', function () {
    ...
    $('.on-table-order-in').on('click', '.on-button-update', function () {
        var row = _table.row($(this).parents('tr')).data();
    ...
    $('.on-table-order-in').on('click', '.on-button-delete', function () {
        var row = _table.row($(this).parents('tr')).data();
    ...
    $('.on-table-order-in tbody').on('click', 'tr td .on-button-not-okay', function () {
        var row = _table.row($(this).parents('tr')).data();
        $.ajax({
            type: 'POST',
            url: '/finance/order-in/update',
            data: {data: 'situation_order_in=0', q: row[ 0 ]},
            success: function (e) {
    ...
    $('.on-table-order-in tbody').on('click', 'tr td .on-button-okay', function () {
        var row = _table.row($(this).parents('tr')).data();

        $.ajax({
            type: 'POST',
            url: '/finance/order-in/update',
            data: {data: 'situation_order_in=1', q: row[ 0 ]},
            success: function (e) {
                UIkit.notification(e.message, {status: e.status});
                switch (e.status) {
                    case 'success':
    ...
    $('.on-button-filter-period').dropdown({
        onChange: function (value) {
            $.ajax({
                type: 'POST',
                url: '/finance/order-in/filter-period',
    ...
    $(document).on('click', '.on-button-filter-period-prev', function () {
        $.ajax({
            type: 'POST',
            url: '/finance/order-in/filter-period-prev',
            success: function (a) {
                switch (parseInt(a.id_filter_period)) {
                    case 1:
                        $('.on-button-filter-period span.text').html(moment(a.filter_period_min).format("DD MMM YYYY"));
    ...
    $(document).on('click', '.on-button-filter-period-next', function () {
        $.ajax({
            type: 'POST',
            url: '/finance/order-in/filter-period-next',
            success: function (a) {
                switch (parseInt(a.id_filter_period)) {
                    case 1:
                        $('.on-button-filter-period span.text').html(moment(a.filter_period_min).format("DD MMM YYYY"));
    ...
    var _form_filter_period = $('.on-form-filter-period').submit(function (event) {
        ...
        onSuccess: function () {
            $.ajax({
                type: 'POST',
                url: '/finance/order-in/filter-period-custom',
    ...
    $.ajax({
        type: 'GET',
        url: '/finance/order-in/filter-period-state',
        success: function (a) {
            switch (parseInt(a.id_filter_period)) {
                case 1:
                    $('.on-button-filter-period span.text').html(moment(a.filter_period_min).format("DD MMM YYYY"));
