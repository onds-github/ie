...
class Finance_OrderInController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        ...
        $this->view->title_page = 'Receitas';
        $this->view->description_page = 'Lançamentos de entrada';

        $this->view->headLink();
        
        $this->view->headScript()
                ->appendFile('/public/modules/finance/script.order-in.js');
    ...
    public function selectAction() {
        ...
        $result[0]['price_order_in'] = number_format($result[0]['price_order_in'], 2, ',', '.');
    ...
    public function ajaxAction() {
        ...
        foreach ($array as $value) {
            $result[] = array(
                ...
                $value['description_order_in'],
                array(
                    nickname_contact => $value['nickname_contact'],
                    document_contact => $value['document_contact']
                ),
                $value['name_account'],
                $value['date_due_order_in'],
                number_format($value['price_order_in'], 2, ',', '.'),
                $value['situation_order_in']
            );
        }
    ...
    public function insertAction() {
        ...
        if ($data['price_order_in'] != '') {
            $data['price_order_in'] = str_replace('.', '', $data['price_order_in']);
            $data['price_order_in'] = str_replace(',', '.', $data['price_order_in']);
        }
